...
import { dateString, getDayIndex } from "./helper.js";
    ...
    isValidIn(calendar){
        ...
        if(calendar.events[newDate]){
            const event = Object.values(calendar.events[newDate]).find(
                (event) => event.id != this.id && event.end > newStart && event.start < newEnd
            );
            if(event){
            $("#errors").text(
        ...
        (1000*60);
        if(duration < 0){
            $("#errors").text("the start cannot be after end.");
            return false;
        }else if(duration < 30){
            $("#errors").text("Event should be at least 30 minutes");
            return false;
        }
        return true;
    }
